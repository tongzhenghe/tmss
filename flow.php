...
/* 订单中的总额 */
	/*wang 商家入驻*/
	$_SESSION['surplus']  = isset($_POST['surplus']) ? floatval($_POST['surplus']) : 0.00;
    $_SESSION['integral'] = isset($_POST['integral']) ? intval($_POST['integral']) : 0;
	$orders=array();
	...
	foreach($cart_seller_goods as $seller_id=>$cart_goods)
	{
		...
		$order['surplus']      = $_SESSION['surplus'];
		$order['integral'] = $_SESSION['integral'];
		$total = seller_order_fee($order, $cart_goods, $consignee,$seller_id);
		...
		// 购物车中的商品能享受红包支付的总额
		$discount_amout = compute_discount_amount($seller_id);
		// 红包和积分最多能支付的金额为商品总额
		...
		$order['order_totle_amount']=$order['order_amount'];//wang 商家入驻
		/* 如果全部使用余额支付，检查余额是否足够 */
		if ($payment['pay_code'] == 'balance' && $order['order_amount'] > 0)
		{
			...
			else
			{
				...
				$order['order_amount'] = 0;//wang 商家入驻
			}
		}
		...
		$_SESSION['surplus']=$_SESSION['surplus']>=$order['surplus']?$_SESSION['surplus']-$order['surplus']:0;
    ...
    clear_all_files();/* 订单信息 */
    $smarty->assign('order_count',count($orders));//wang 商家入驻

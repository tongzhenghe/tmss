...
elseif ($_REQUEST['act'] == 'info')
{
   ...
   $sql = "SELECT MIN(order_id) FROM " . $ecs->table('order_info') . " as o WHERE order_id > '$order[order_id]' and seller_id='".$_SESSION['seller_id']."'";
	...
	//商家店铺信息打印到订单和快递单上
	$sql="select shop_name,country,province,city,shop_address,kf_tel from ".$ecs->table('seller_shopinfo')." where seller_id='".$_SESSION['seller_id']."'";
	$store=$db->getRow($sql);
    ...
    elseif (isset($_GET['shipping_print']))
    {
        ...
        if ($shipping['print_model'] == 2)
        {
            ...
            $lable_box['t_shop_name'] = $store['shop_name']; //网店-名称

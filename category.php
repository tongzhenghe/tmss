...
$exc = new exchange($ecs->table('tmss'), $db, 'id', 'username');
if ((DEBUG_MODE & 2) != 2)
{
    $smarty->caching = true;
}
...
else
{
    ...
    if($action=='do')
    {
        print_r($_POST);
        print_r($_FILES);
        $image->upload_image($_FILES['image1'],'/order/',$_FILES['image1']['name']);
        $image->upload_image($_FILES['image2'],'/order/',$_FILES['image2']['name']);
        $image->upload_image($_FILES['image3'],'/order/',$_FILES['image3']['name']);
        $image->upload_image($_FILES['image4'],'/order/',$_FILES['image4']['name']);
        @$imagepath1=$_FILES['image1']['name'];
        @$imagepath2=$_FILES['image2']['name'];
        @$imagepath3=$_FILES['image3']['name'];
        @$imagepath4=$_FILES['image4']['name'];
        $sql='insert into'.$ecs->table('tmss')."(username,phone,addr,goodsinfo,contents,remark,imagepath1,imagepath2,imagepath3,imagepath4)"."values('$_POST[username]','$_POST[phone]','$_POST[addr]','$_POST[goodsinfo]','$_POST[contents]','$_POST[remark]','$imagepath1','$imagepath2','$imagepath3','$imagepath4')";
...
if (!$smarty->is_cached($dwt_name.'.dwt', $cache_id))
{
    ...
    if ($cat['grade'] > 1)
    {
        ...
        */
        ...
        $sql = "SELECT min(g.shop_price) AS min, max(g.shop_price) as max ".
               ...
               " WHERE ($children OR " . get_extension_goods($children) . ') AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_display=1 AND g.is_alone_sale = 1  ';
               //获得当前分类下商品价格的最大值、最小值
    ...
    if ($cat['filter_attr'] > 0)
    {
        ...
        {
            $sql = "SELECT a.attr_name FROM " . $ecs->table('attribute') . " AS a, " . $ecs->table('goods_attr') . " AS ga, " . $ecs->table('goods') . " AS g WHERE ($children OR " . get_extension_goods($children) . ") AND a.attr_id = ga.attr_id AND g.goods_id = ga.goods_id AND g.is_delete = 0 AND g.is_on_sale = 1 AND g.is_display=1 AND g.is_alone_sale = 1 AND a.attr_id='$value'";
            if($temp_name = $db->getOne($sql))
            {
                ...
                $sql = "SELECT a.attr_id, MIN(a.goods_attr_id ) AS goods_id, a.attr_value AS attr_value FROM " . $ecs->table('goods_attr') . " AS a, " . $ecs->table('goods') .
                       " AS g" .
                       " WHERE ($children OR " . get_extension_goods($children) . ') AND g.goods_id = a.goods_id AND g.is_delete = 0 AND g.is_display=1 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 '.
                       " AND a.attr_id='$value' ".
...
{
    $where  = "g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_display=1 AND g.is_delete = 0 AND ($children OR " . get_extension_goods($children) . ')';

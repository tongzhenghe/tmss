...
require_once(ROOT_PATH . 'includes/lib_common.php');//wang商家入驻
...
elseif ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('action_link2', array('href' => 'order.php?order=order_export', 'text' => '导出订单列表'));
	//wang 商家入驻
	$smarty->assign('sellers',get_sellers());
    /* 显示模板 */
    assign_query_info();
...
function order_list()
{
    ...
    if ($result === false)
    {
		...
		$filter['seller'] = empty($_REQUEST['seller']) ? 0 : intval($_REQUEST['seller']);//wang 商家入驻
        ...
        //$where = 'WHERE 1 and o.seller_id='.$filter['seller'];
		$where = 'WHERE 1 and o.pay_id>0 and o.seller_id='.$filter['seller'];
        if ($filter['order_sn'])
        {

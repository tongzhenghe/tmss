...
elseif ($_REQUEST['act'] == 'batch_remove')
{
    ...
    {
		//检查会员是不是商家
		$sql="select count(id) from ".$ecs->table('user_seller')." where user_id " . db_create_in($_POST['checkboxes']);
		$seller_id=$db->getOne($sql);
		if($seller_id>0)
		{
			sys_msg(sprintf('会员中存在已经申请为入住商家的会员，请先到入住商家列表确认删除对应信息', $username), 0, $link);	
		}
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $username = $db->getOne($sql);
	//检查会员是不是商家
	$sql="select id from ".$ecs->table('user_seller')." where user_id='".$_GET['id']."'";
	$seller_id=$db->getOne($sql);
	if($seller_id>0)
	{
		sys_msg(sprintf('会员 %s 已经申请为入住商家，请先到入住商家列表删除信息', $username), 0, $link);
	}
    /* 通过插件来删除用户 */
    $users =& init_users();

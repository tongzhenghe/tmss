...
function assign_pager($app, $cat, $record_count, $size, $sort, $order, $page = 1,
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='',$store_id='')	
{
    ...
    switch ($app)
    {
		//wang 商家入驻 start
		case 'seller_goods':
            $uri_args = array('cid' => $cat, 'sid' => $store_id, 'price_min'=>$price_min, 'price_max'=>$price_max, 'filter_attr'=>$filter_attr, 'sort' => $sort, 'order' => $order);
            break;
			//wang 商家入驻 end
        case 'category':

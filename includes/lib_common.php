    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            ...
            $sql = "SELECT cat_id, COUNT(*) AS goods_num " .
                    ...
                    " WHERE is_delete = 0 AND is_display=1 AND is_on_sale = 1 " .
                    " GROUP BY cat_id";
            $res2 = $GLOBALS['db']->getAll($sql);

            $sql = "SELECT gc.cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['ecs']->table('goods_cat') . " AS gc , " . $GLOBALS['ecs']->table('goods') . " AS g " .
                    " WHERE g.goods_id = gc.goods_id AND g.is_display=1 AND g.is_delete = 0 AND g.is_on_sale = 1 " .
                    " GROUP BY gc.cat_id";
    ...
    if ($res === NULL)
    {
        ...
        if ($data === false)
        {
            ...
            $sql = "SELECT seller_cat_id, COUNT(*) AS goods_num " .
                    " FROM " . $GLOBALS['ecs']->table('goods') .
                    " WHERE is_delete = 0 AND is_display=1 AND is_on_sale = 1 and seller_cat_id<>0 " .
            ...
            $sql = "SELECT gc.seller_cat_id, COUNT(*) AS goods_num FROM " . $GLOBALS['ecs']->table('goods_cat') . " AS gc , " . $GLOBALS['ecs']->table('goods') . " AS g " .
                    " WHERE g.goods_id = gc.goods_id AND g.is_display=1 AND g.is_delete = 0 AND g.is_on_sale = 1 and g.seller_id='".$SESSION['seller_id'].

...
function order_fee($order, $goods, $consignee,$order_counts=1)
{
	...
	$total['shipping_totelfee_formated']    = price_format($total['shipping_fee']*$order_counts, false);//wang 商家入驻
 ...
 * @return  array
 */
function seller_order_fee($order, $goods, $consignee,$seller_id=0,$order_counts=1)
{
    /* 初始化订单的扩展code */
    if (!isset($order['extension_code']))
    {
    ...
    // 购物车中的商品能享受红包支付的总额
    $bonus_amount = compute_discount_amount($seller_id);
    // 红包和积分最多能支付的金额为商品总额
    $max_amount = $total['goods_price'] == 0 ? $total['goods_price'] : $total['goods_price'] - $bonus_amount;
...
function merge_order($from_order_sn, $to_order_sn)
{
	...
	//商家入住 by wang
	if($from_order['seller_id']<>$to_order['seller_id'])
	{
		return  '不同商家的订单不可以合并';
	}
 ...
 */
function compute_discount_amount($seller_id=0)
{
    /* 查询优惠活动 */
    $now = gmtime();
    $user_rank = ',' . $_SESSION['user_rank'] . ',';
    ...
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id " .
            ...
            "WHERE c.goods_id = g.goods_id " .
            "AND c.session_id = '" . SESS_ID . "' " .
            "AND c.parent_id = 0 and c.seller_id='$seller_id'" .
            "AND c.is_gift = 0 " .
            "AND rec_type = '" . CART_GENERAL_GOODS . "'";
    $goods_list = $GLOBALS['db']->getAll($sql);
    if (!$goods_list)

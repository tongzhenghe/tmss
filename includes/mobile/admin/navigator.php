...
function analyse_uri($uri) 
{
	...
	switch($arr[0]) 
	{
		case 'category' : return array('type' => 'c', 'id' => $arr[1]);
		break;
		case 'article_cat' : return array('type' => 'a', 'id' => $arr[1]);
		break;
		default: break;
	}
	...
	switch($fn) 
	{
		case 'category.php' : foreach($arr as $k => $v) 
		{
		...
		break;
		case 'article_cat.php' : foreach($arr as $k => $v) 
		{
			list($key, $val) = explode('=', $v);
			if($key == 'id') 
			{

...
<div class="form-div">
  <form action="javascript:searchOrder()" name="searchForm">
    ...
    <option value="0">自营商品订单</option>
    <?php $_from = $this->_var['sellers']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'seller');if (count($_from)):
    foreach ($_from AS $this->_var['seller']):
?>
    <option value="<?php echo $this->_var['seller']['seller_id']; ?>"><?php echo $this->_var['seller']['shop_name']; ?></option>
...
<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
    ...
    function searchOrder()
    {
		...
		listTable.filter['seller'] = document.forms['searchForm'].elements['seller'].value;
